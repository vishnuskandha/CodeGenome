...
class AnalysisResult:
    """Complete analysis result (v2 enhanced)"""
    repo_url: str
...
class GitHubFetcher:
    ...
    def __init__(self, repo_url: str, token: Optional[str] = None):
        ...
        # Ignore placeholder tokens to avoid 401 errors
        if self.token and ("paste_your_token" in self.token or self.token == "your_token_here"):
            self.token = None
    ...
    def fetch_code_files(self, max_files: int = 200) -> List[FileInfo]:
        ...
        for item in tree:
            ...
            # Priority 1: Check if it's a LICENSE file (before extension check)
            if filename in ['license', 'license.md', 'license.txt', 'licence', 'licence.md']:
                console.print(f"[dim]Fetching {path} (LICENSE)...[/dim]")
                content = self.get_file_content(path)
                if content:
                    license_files.append(FileInfo(
                        path=path,
                        content=content,
                        size=item["size"],
                        language="text"
                    ))
            # Priority 2: Check if it's a README (before extension check)
            ...
            # Priority 3: Check if it's a code file by extension
            elif ext.lower() in LANGUAGE_MAP:
        ...
        # Combine all files
        all_files = code_files + readme_files + license_files
        console.print(f"[green]Fetched {len(code_files)} code files + {len(readme_files)} README files + {len(license_files)} LICENSE files[/green]")
        return all_files
...
# ============================================================================
# MODULE 4.5: GIT ANALYZER (v2)
# ============================================================================

class GitAnalyzer:
    """Analyzes Git history for commit patterns and hotspots"""
    
    def __init__(self, repo_url: str):
        self.repo_url = repo_url
        self.repo_path = None
        
    def clone_shallow(self, depth: int = 50) -> bool:
        """Shallow clone for history analysis"""
        if not GIT_AVAILABLE:
            console.print("[yellow]GitPython not available. Skipping Git analysis.[/yellow]")
            return False
    ...
    def analyze_commits(self) -> Dict[str, Any]:
        ...
        try:
            ...
            hotspots = sorted(file_changes.items(), key=lambda x: x[1], reverse=True)[:10]
            ...
            console.print(f"[yellow]Commit analysis failed: {e}[/yellow]")
            return {}


# ============================================================================
# MODULE 4.6: QUALITY ENGINE (v2)
# ============================================================================
...
class QualityEngine:
    ...
    @staticmethod
    def calculate_quality_score(files: List[FileInfo], metadata: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """Calculate comprehensive quality score"""
        
        # 1. Documentation coverage
        total_functions = sum(len(m["functions"]) for m in metadata.values())
        documented_functions = 0
        
        for file_info in files:
            if file_info.language == "python":
                # Simple docstring detection
                documented_functions += file_info.content.count('"""')
        
        doc_coverage = (documented_functions / max(total_functions, 1)) * 100
        
        # 2. Naming conventions (simple heuristic)
        naming_score = 100
        for file_info in files:
            # Check for snake_case violations in Python
            if file_info.language == "python":
                if re.search(r'def [A-Z]', file_info.content):  # CamelCase function
                    naming_score -= 5
        
        # 3. Complexity score (already calculated)
        avg_complexity = sum(m["complexity"] for m in metadata.values()) / max(len(metadata), 1)
        ...
        test_coverage_estimate = (len(test_files) / max(len(files), 1)) * 100
...
# ============================================================================
# MODULE 4.7: DEPENDENCY SCANNER (v2)
# ============================================================================
...
class DependencyScanner:
    ...
    @staticmethod
    def scan_requirements(files: List[FileInfo]) -> List[Dict[str, Any]]:
        """Scan requirements.txt and package.json for CVEs"""
        vulnerabilities = []
            ...
            if file_info.path.endswith('requirements.txt'):
                ...
                for line in file_info.content.split('\n'):
                    ...
                    if line and not line.startswith('#'):
                        ...
                        # Note: Real CVE check would use OSV API here
                        # For now, just detect outdated patterns
                        if 'django==1.' in line.lower() or 'flask==0.' in line.lower():
                            vulnerabilities.append({
                                "file": file_info.path,
                                "package": pkg,
                                "issue": "Potentially outdated package version",
                                "severity": "medium"
                            })
            
            elif file_info.path.endswith('package.json'):
                # Parse npm dependencies
                try:
                    pkg_data = json.loads(file_info.content)
...
# ============================================================================
# MODULE 4.8: LICENSE CHECKER (v2)
# ============================================================================

class LicenseChecker:
    """Detects and validates licenses"""
    ...
    @staticmethod
    def detect_license(files: List[FileInfo]) -> Dict[str, Any]:
        """Detect license from LICENSE file"""
        for file_info in files:
            if 'license' in file_info.path.lower():
                content = file_info.content
                
                for license_name, pattern in LicenseChecker.LICENSE_PATTERNS.items():
...
class RiskEngine:
    ...
    def calculate_risk_score(
        ...
        circular_deps: int,
        files: List[FileInfo] = None  # v2: Added to detect doc-only repos
    ) -> float:
        """
        Calculate risk score (0-100)
        Formula: (critical×5 + high×3 + medium×2 + low×1) + (avg_complexity/10) + (circular_deps×2)
        
        v2: Handles documentation-only repositories
        """
        severity_counts = {"critical": 0, "high": 0, "medium": 0, "low": 0}
        for issue in security_issues:
            severity_counts[issue.severity] += 1
        
        # v2: Check if this is a documentation-only repo
        if files:
            doc_languages = {'markdown', 'text', 'json', 'yaml', 'xml'}
            doc_files = [f for f in files if f.language in doc_languages]
            is_doc_only = len(doc_files) / max(len(files), 1) > 0.8
            
            if is_doc_only and len(security_issues) == 0:
                # Documentation-only repo with no security issues = low risk
                return 5.0
...
class LLMReasoner:
    """Interfaces with OpenRouter or OpenAI for AI-powered analysis"""
    
    def __init__(self, api_key: Optional[str] = None, model: Optional[str] = None):
        # Try to get API key from multiple sources (OpenRouter prioritized for requested GPT OSS 120B)
        self.api_key = api_key or os.getenv("OPENROUTER_API_KEY") or os.getenv("SAMBANOVA_API_KEY") or os.getenv("OPENAI_API_KEY")
        
        if not self.api_key:
            raise ValueError(
                "No API key found. Please set one of the following in your .env file:\n"
                "  - SAMBANOVA_API_KEY (for SambaNova Cloud)\n"
                "  - OPENROUTER_API_KEY (for OpenRouter)\n"
                "  - OPENAI_API_KEY (for OpenAI)"
            )
        
        # Auto-detect provider based on API key prefix or format
        if self.api_key.startswith("sk-or-"):
            # OpenRouter key
            self.provider = "openrouter"
            self.base_url = "https://openrouter.ai/api/v1"
            self.model = model or "openai/gpt-oss-120b"  # User requested GPT OSS 120B
            console.print("[dim]Using OpenRouter API with GPT OSS 120B model[/dim]")
        ...
        elif os.getenv("SAMBANOVA_API_KEY") == self.api_key or (len(self.api_key) == 36 and self.api_key.count("-") == 4):
            # SambaNova key (typically UUID format)
            self.provider = "sambanova"
            self.base_url = "https://api.sambanova.ai/v1"
            self.model = model or "Meta-Llama-3.1-70B-Instruct"
            console.print("[dim]Using SambaNova Cloud API[/dim]")
        else:
            # Default to OpenRouter for unknown key formats
            self.provider = "openrouter"
            self.base_url = "https://openrouter.ai/api/v1"
            self.model = model or "openai/gpt-oss-120b"  # User requested GPT OSS 120B
            console.print("[dim yellow]Warning: Unknown API key format, defaulting to OpenRouter[/dim yellow]")
        
        # Initialize OpenAI client with detected settings
        self.client = OpenAI(
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You are a senior software architect analyzing code in a CLI terminal. "
                    "Provide extremely concise, technical, and objective answers. "
                    "Use simple markdown: bolding, lists, and code blocks. "
                    "DO NOT use emojis. DO NOT use wide tables (they break in CLI windows); use bulleted lists instead. "
                    "Keep total output under 150 words unless specifically asked for deep detail."
                )
            },
            {
                "role": "user",
...
class ChatInterface:
    ...
    def display_summary(self):
        """Display analysis summary with v2 enhancements"""
        console.print("\n")
        console.print(Panel.fit(
            f"[bold cyan]CodeGenome v2 Analysis Complete[/bold cyan]\n"
            f"Repository: {self.analysis.repo_url}",
            border_style="cyan"
        ))
        
        # Architecture table
        arch_table = Table(title="Architecture Summary", show_header=True, border_style="cyan")
        arch_table.add_column("Property", style="cyan", width=18)
        ...
        arch_table.add_row("Total Lines", f"{self.analysis.total_lines:,}")
        
        console.print(arch_table)
        
        # v2: Git Analysis Table
        if self.analysis.git_analysis and self.analysis.git_analysis.get("total_commits"):
            git_table = Table(title="Git History Analysis", show_header=True, border_style="blue")
            git_table.add_column("Metric", style="cyan", width=18)
            git_table.add_column("Value", style="yellow")
            
            git = self.analysis.git_analysis
            git_table.add_row("Total Commits", str(git.get("total_commits", 0)))
            git_table.add_row("Contributors", str(git.get("contributors", 0)))
            
            # Top contributors
            top_contribs = git.get("top_contributors", [])
            if top_contribs:
                top_3 = ", ".join([f"{name} ({count})" for name, count in top_contribs[:3]])
                git_table.add_row("Top Contributors", top_3)
            
            # Hotspots
            hotspots = git.get("hotspots", [])
            if hotspots:
                top_hotspot = f"{hotspots[0][0]} ({hotspots[0][1]} changes)"
                git_table.add_row("Top Hotspot", top_hotspot)
            
            console.print(git_table)
        
        # v2: Code Quality Table
        if self.analysis.quality_metrics:
            quality = self.analysis.quality_metrics
            quality_score = quality.get("quality_score", 0)
            quality_color = "green" if quality_score >= 70 else "yellow" if quality_score >= 50 else "red"
            
            quality_table = Table(title="Code Quality Metrics", show_header=True, border_style="green")
            quality_table.add_column("Metric", style="cyan", width=22)
            quality_table.add_column("Score", style=quality_color)
            
            quality_table.add_row("Overall Quality", f"{quality_score}/100")
            quality_table.add_row("Documentation Coverage", f"{quality.get('doc_coverage', 0):.1f}%")
            quality_table.add_row("Naming Conventions", f"{quality.get('naming_score', 0):.1f}%")
            quality_table.add_row("Complexity Score", f"{quality.get('complexity_score', 0):.1f}%")
            quality_table.add_row("Test Coverage Est.", f"{quality.get('test_coverage_estimate', 0):.1f}%")
            quality_table.add_row("Test Files Found", str(quality.get('test_files_found', 0)))
        ...
        sec_table = Table(title="Security Analysis", show_header=True, border_style=risk_color)
        sec_table.add_column("Metric", style="cyan", width=18)
        ...
        # v2: Dependency Vulnerabilities
        if self.analysis.dependency_vulns:
            console.print("\n[bold yellow]Dependency Vulnerabilities:[/bold yellow]")
            for vuln in self.analysis.dependency_vulns[:5]:
                severity_marker = "[CRITICAL]" if vuln['severity'] == 'high' else "[WARNING]" if vuln['severity'] == 'medium' else "[INFO]"
                console.print(f"{severity_marker} {vuln['file']}: {vuln['package']} - {vuln['issue']}")
        
        # v2: License Info
        if self.analysis.license_info and self.analysis.license_info.get("license"):
            license_name = self.analysis.license_info["license"]
            license_status = "[OK]" if self.analysis.license_info.get("compatible") else "[WARNING]"
            console.print(f"\n[bold magenta]License:[/bold magenta] {license_status} {license_name}")
        ...
        if self.analysis.security_issues:
            ...
            for idx, issue in enumerate(self.analysis.security_issues[:5], 1):
                ...
                console.print(f"[cyan]Problem:[/cyan] {issue.description}")
...
def analyze_repository(repo_url: str, max_files: int = 200) -> Tuple[AnalysisResult, Dict[str, Dict[str, Any]], LLMReasoner]:
    ...
    # Step 6: Risk scoring
    risk_score = RiskEngine.calculate_risk_score(security_issues, avg_complexity, len(cycles), files)
    ...
    quality_metrics = QualityEngine.calculate_quality_score(files, metadata)
    
    # Step 9 (v2): Dependency Vulnerability Scan
    console.print("[cyan]Scanning dependencies for vulnerabilities...[/cyan]")
    dependency_vulns = DependencyScanner.scan_requirements(files)
    
    # Step 10 (v2): License Detection
    console.print("[cyan]Detecting license...[/cyan]")
    license_info = LicenseChecker.detect_license(files)
...
def main():
    """Main entry point"""
    console.print(Panel.fit(
        "[bold cyan]CodeGenome v2[/bold cyan]\n"
        "[dim]Multi-Dimensional Codebase Intelligence Engine[/dim]\n\n"
        "Git History | Quality Scoring | Security | License Detection",
        border_style="cyan",
